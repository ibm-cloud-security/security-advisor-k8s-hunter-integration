...
def delete_notes(account_id, token, endpoint, notes):
    ...
    for note in notes:
        if note['kind'] == "CARD":
            url = endpoint + "/" + account_id + "/providers/kubeHunterRedhatOpenshift/notes/"+ note['id']
        elif note['provider_id'] == "kubeHunterRedhatInformationDisclosure":
            url = endpoint + "/" + account_id + "/providers/kubeHunterRedhatInformationDisclosure/notes/"+ note['id']
        elif note['provider_id'] == "kubeHunterRedhatRemoteCodeExecutor":
            url = endpoint + "/" + account_id + "/providers/kubeHunterRedhatRemoteCodeExecutor/notes/"+ note['id']

        elif note['provider_id'] == "kubeHunterRedhatIdentityAndAccess":
            url = endpoint + "/" + account_id + "/providers/kubeHunterRedhatIdentityAndAccess/notes/"+ note['id']
        elif note['provider_id'] == "kubeHunterRedhatDenialofService":
            url = endpoint + "/" + account_id + "/providers/kubeHunterRedhatDenialofService/notes/"+ note['id']
        try:
